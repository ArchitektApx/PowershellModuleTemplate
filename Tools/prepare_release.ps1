# Prepares a release: gates on lint/build/test, verifies the changelog has content for the
# release, stamps the new version into README.md and build.psd1, then rebuilds and verifies
# the built module actually carries the new version.
#
# Usage (from anywhere):
...
# and tag v<Version> on the default branch.
...
try {
  ...
  if (Test-ChangelogSectionContent $versionBody) {
    ...
    # Promote the Unreleased section to this release, leaving an empty [Unreleased] above it
    # so the next change has somewhere to go and the [Unreleased] link ref still resolves.
  ...
  $builtManifest = Join-Path (Join-Path $info.DistRoot $Version) "$($info.ModuleName).psd1"
  ...
  Write-Host "Next: review the diff, commit, then tag v$Version on the default branch and push the tag."
} finally {
  Pop-Location
}
